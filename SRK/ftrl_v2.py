...
class ftrl(object):
	...
	def fit(self,line):
		...
		#print len(line)
		if self.interaction:
			temp_X = [0.] * (len(line)*(len(line)-1)/2)
			index_counter = 0 
			for i, key in enumerate(line):
				for j,key1 in enumerate(line):
					if j>i:
						val,val1 = line[key],line[key1]
						temp_X[index_counter] = (abs(hash(key + '_' + val+'_'+key1 + '_' + val1)) % self.bits)
						index_counter += 1
...
if __name__ == '__main__':
	...
	"""
	path='../../data/'
	train = path+'trainSearchStream.tsv'

	# craeting a dict based on ads info #
	ads_dict= {}
	ads_reader = DictReader(open(path+"AdsPreProcessed.tsv"))
	...
	test = path+'testSearchStream.tsv'
	except_count = 0 
