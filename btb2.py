...
if __name__ == '__main__':
	...
	with open('btb2.csv', 'w') as output:
                output.write('ID,IsClick\n')
		for t, line in enumerate(DictReader(open(test), delimiter='\t')):
			idx=line['ID']
	  		if line['ObjectType']!='3':
                            	continue
        		clf.fit(line)
                        output.write(idx+','+str(clf.predict())+'\n')
                        
			#output.write('%s,%s\n' % str(clf.predict()))
        """
	sample = pd.read_csv('../../data/sampleSubmission_HistCTR.csv')
